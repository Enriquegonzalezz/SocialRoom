  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        const emailResponse = await fetch(`${baseUrl}/api/send-lead-email`, {
          method: 'POST',
          headers: {
        ...
        if (!emailResponse.ok) {
          console.error('❌ Error sending email notification:', emailData);
        } else {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-start">
            ...
            <div className='flex flex-col items-center lg:items-start last:mx-auto px-4'>
              <h2 className="text-5xl sm:text-5xl md:text-7xl lg:text-7xl font-light mb-6 mt-10 md:mb-10 text-[#f4f4f4] font-thermal">
                ...
                <a
                  href="https://wa.me/+584120639249"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-4 inline-flex items-center justify-center min-h-10 px-8 py-4 border border-black text-[0.75rem] font-normal leading-[130%] tracking-[0.03em] uppercase bg-white text-black cursor-pointer select-none transition duration-150 ease-out hover:bg-black hover:text-white"
                  style={{
                ...
                >
                  <span className="underline-offset-4 decoration-current underline font-helvetica text-[14px]">
              ...
              <div className="relative z-10 bg-[#D92C01] p-8 w-full max-w-[550px] shadow-2xl">
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-4">
                  ...
                  <button 
                    ...
                    className="inline-flex items-center justify-center w-full min-h-10 px-4 border border-black text-[0.75rem] font-normal leading-[130%] tracking-[0.03em] uppercase bg-white text-black cursor-pointer select-none transition duration-150 ease-out hover:bg-black hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                    style={{
                      boxShadow: '0 0 0 0 #aaaaac',
                      transition: 'background-color .15s ease, border-color .15s ease, color .15s ease, box-shadow .15s ease-out',
                    }}
                  >
                    <span className="underline-offset-4 decoration-current underline font-helvetica text-[14px]">
                      {isSubmitting ? t('contact.send') + '...' : t('contact.startCreating')}
                    </span> 
                    {!isSubmitting && (
                      <ArrowOutwardIcon style={{ width: 32, height: 32, marginLeft: '8px' }} />
                    )}
                  </button>
                </form>
              </div>
